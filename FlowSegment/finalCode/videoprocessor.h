...
class FrameProcessor {
  ...
  public:
	// processing method
	virtual void process(cv::Mat &input, cv::Mat& lastInput, cv:: Mat &output)= 0;
};
...
class VideoProcessor {
  ...
  private:
	  ...
	  // process就是一个函数指针名，这个函数指针的类型
	  // 是接受两个cv::Mat类型的引用，返回void
  ...
  public:
	  ...
	  // 设置处理帧的回调函数，就是这个函数指针可以指向不同的图像处理算法
	  // 这个其实就是对应了stackoverflow的那个答案里说的回调函数的目的之一
	  // 就是更generci的code，这个调用的函数可以使用不同的回调函数
	  ...
	  void run() {
		  ...
		  while (!isStopped()) {
			  ...
			  if (callIt) {
				...
				// process the frame
				if (process)
				    process(frame, frames[0], output);
				else if (frameProcessor) 
					frameProcessor->process(frame, frames[0], output);
